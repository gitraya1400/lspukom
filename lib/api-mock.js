...
allUnitsDb.set(
  ...
  SKEMA_ADS_UNITS.map((unit, idx) => ({
    ...
    materiCount: unit.nomorUnit === 1 ? 3 : 0,
    soalCount: 2,
...
export async function mockAddYearToSkema(
  ...
  if (duplikasiDariTahun) {
    ...
    sourceUnits.forEach((oldUnit, idx) => {
      ...
      if (materiLama && materiLama.length > 0) {
        const newMateri = materiLama.map((m) => ({
          ...m,
          id: `MAT-${newUnitId}-${Date.now()}${Math.random()
            .toString(36)
            .slice(2, 6)}`,
          unitId: newUnitId,
        }));
      ...
      if (soalLama && soalLama.length > 0) {
        const newSoalList = soalLama.map((s) => ({
          ...s,
          id: `SOAL-${newUnitId}-${Date.now()}${Math.random()
            .toString(36)
            .slice(2, 6)}`,
          unitId: newUnitId,
          tahunAjaran: tahunBaru,
    ...
    if (typeof globalThis !== "undefined" && globalThis.allMateriDb)
      globalThis.allMateriDb = materiDb;
    if (typeof globalThis !== "undefined" && globalThis.allSoalDb)
      globalThis.allSoalDb = soalDb;
    ...
    praktikumList = [
      ...
      ...praktikumToCopy.map((p) => ({
        ...p,
        id: `${skemaId}-PRAKTIKUM-${tahunBaru.replace(
          /\D/g,
          ""
  ...
  try {
    if (typeof globalThis !== "undefined") {
      ...
      if (fallback.length > 0) {
        ...
        globalThis.allMateriDb.set(unitId, []);
      }
