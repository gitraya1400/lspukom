...
function generateAsesiData() {
  ...
  const adsKelasList = ["3D31", "3D32", "3D33"];
...
// --- Simpan tahun ajaran per skema (gunakan Map jika belum ada) ---
...
/**
 * Mendapatkan daftar tahun ajaran untuk skema tertentu
 */
...
export async function mockCreateSkema(skemaData) {
  ...
  // inisialisasi tahunAjaranDb & praktikum untuk skema baru
  if (typeof tahunAjaranDb !== "undefined") tahunAjaranDb.set(newSkema.id, new Set(["2024/2025"]));
  if (typeof allPraktikumSoalDb !== "undefined") allPraktikumSoalDb.set(newSkema.id, []);
  if (typeof globalThis !== "undefined") {
    if (!globalThis.allPraktikumSoalDb) globalThis.allPraktikumSoalDb = allPraktikumSoalDb;
    globalThis.allPraktikumSoalDb.set(newSkema.id, globalThis.allPraktikumSoalDb.get(newSkema.id) || []);
  }
...
export async function mockAddYearToSkema(skemaId, tahunBaru, duplikasiDariTahun = null) {
  ...
  // --- Sinkronkan ke allSkemaDb supaya UI / delete memakai sumber data yang sama ---
  const skema = allSkemaDb.get(skemaId);
  if (skema) {
    skema.tahunAjaranList = Array.from(new Set([...(skema.tahunAjaranList || []), tahunBaru]));
    allSkemaDb.set(skemaId, skema);
  }

  // Jika diminta duplikasi dari tahun tertentu
  if (duplikasiDariTahun) {
    const units = allUnitsDb.get(skemaId) || [];

    // pilih unit yang berasal dari tahun duplikasi
    const sourceUnits = units.filter(u => u.tahunAjaran === duplikasiDariTahun);

    // buat salinan unit dengan id baru (unik) dan tahun baru
    const unitsToCopy = sourceUnits.map((u) => {
      ...
      return {
        ...
        // jangan bawa urutan persis — set ke akhir nanti
      };
    });

    // gabungkan units (tetap mempertahankan units lama)
    const mergedUnits = [...units, ...unitsToCopy];
    allUnitsDb.set(skemaId, mergedUnits);

    // copy materi & soal per unit: gunakan oldUnit.id (u.id) sebagai kunci sumber
    const materiDb = (typeof globalThis !== "undefined" && globalThis.allMateriDb) ? globalThis.allMateriDb : new Map();
    const soalDb = (typeof globalThis !== "undefined" && globalThis.allSoalDb) ? globalThis.allSoalDb : allSoalDb;

    sourceUnits.forEach((oldUnit, idx) => {
      const newUnit = unitsToCopy[idx];
      const oldUnitId = oldUnit.id;
      const newUnitId = newUnit.id;

      // duplicate materi (jika ada)
      const materiLama = materiDb.get(oldUnitId) || [];
      if (materiLama && materiLama.length > 0) {
        const newMateri = materiLama.map(m => ({ ...m, id: `MAT-${newUnitId}-${Date.now()}${Math.random().toString(36).slice(2,6)}`, unitId: newUnitId }));
        materiDb.set(newUnitId, newMateri);
      }

      // duplicate soal unit (jika ada)
    ...
    // simpan kembali materiDb/soalDb ke global jika ada
    if (typeof globalThis !== "undefined" && globalThis.allMateriDb) globalThis.allMateriDb = materiDb;
    if (typeof globalThis !== "undefined" && globalThis.allSoalDb) globalThis.allSoalDb = soalDb;

    // Duplikasi Praktikum (tetap seperti sebelumnya)
    let praktikumList = allPraktikumSoalDb.get(skemaId) || [];
    const praktikumToCopy = praktikumList.filter(p => p.tahunAjaran === duplikasiDariTahun);
    praktikumList = [
      ...praktikumList,
      ...praktikumToCopy.map(p => ({
        ...p,
        id: `${skemaId}-PRAKTIKUM-${tahunBaru.replace(/\D/g,'')}-${Date.now()}${Math.random().toString(36).slice(2,6)}`,
        tahunAjaran: tahunBaru,
      }))
    ];
...
export async function mockDeleteYearFromSkema(skemaId, tahunAjaran) {
  ...
  // Hapus Unit yang punya tahun tersebut
  const units = allUnitsDb.get(skemaId) || [];
  ...
  // Bersihkan data anak (Soal & Materi)
...
export async function mockDeleteTahunAjaran(skemaId, tahunTarget) {
  await delay(800);
  const skema = allSkemaDb.get(skemaId);
  if (!skema) throw new Error("Skema tidak ditemukan");

  skema.tahunAjaranList = skema.tahunAjaranList.filter(t => t !== tahunTarget);
  allSkemaDb.set(skemaId, skema);

  // Hapus Unit & Soal terkait tahun
  const units = allUnitsDb.get(skemaId) || [];
  const unitsToDelete = units.filter(u => u.tahunAjaran === tahunTarget);
  const remainingUnits = units.filter(u => u.tahunAjaran !== tahunTarget);
  ...
  // Hapus Praktikum terkait tahun
  const praktikumList = allPraktikumSoalDb.get(skemaId) || [];
  const remainingPraktikum = praktikumList.filter(p => p.tahunAjaran !== tahunTarget);
  allPraktikumSoalDb.set(skemaId, remainingPraktikum);
...
export async function mockGetSoalTryoutGabungan(skemaId, tahunAjaran) {
  ...
  const filteredUnits = tahunAjaran ? unitsInSkema.filter(u => u.tahunAjaran === tahunAjaran) : unitsInSkema;
  let allTryoutSoal = [];
...
export async function mockCreateUnit(skemaId, unitData) {
  ...
  // Filter unit HANYA di tahun yang dipilih untuk cek nomor urut
  const unitsInYear = units.filter(u => u.tahunAjaran === unitData.tahunAjaran);
  
  // Auto number berdasarkan tahun tersebut
  const nomor = Number(unitData.nomorUnit) || unitsInYear.length + 1;

  // VALIDASI DUPLIKAT: nomorUnit di tahun yg sama tidak boleh sama
  ...
  // VALIDASI DUPLIKAT: kodeUnit di tahun yg sama tidak boleh sama (jika diberikan)
  ...
  // FIX ID: Tambahkan random string biar ga bentrok antar tahun/edit
...
export async function mockUpdateUnit(skemaId, unitId, updates) {
  ...
  // FIX: Pastikan pencarian ID benar
  ...
  units[idx] = { 
    ...existing, 
    ...updates,
    // Jaga agar ID dan Tahun tidak berubah saat edit
    id: existing.id,
...
export async function mockDeleteSoal(skemaId, soalId) {
  ...
  // gunakan sumber DB praktikum yang konsisten
  const prakDb =
    typeof globalThis !== "undefined" && globalThis.allPraktikumSoalDb
      ? globalThis.allPraktikumSoalDb
      : allPraktikumSoalDb;
...
export async function mockUpdateSoal(unitId, soalId, updates) {
  ...
  // 1. Jika ini soal Praktikum (tidak punya unitId)
  if (updates.tipeSoal === "UJIAN_PRAKTIKUM") {
     // Praktikum tidak disimpan di allSoalDb, tapi di allPraktikumSoalDb (skip logic ini)
     return; 
  }

  // 2. Jika Unit ID diketahui (Ideal)
  if (unitId) {
  ...
  // 3. FIX: Fallback Search (Cari di SEMUA unit jika unitId null/salah)
  // Ini yang bikin error "tidak ditemukan" sebelumnya
...
export async function mockUpsertPraktikum(skemaId, praktikumData) {
  ...
  // validasi server-side: praktikum dengan tipe upload harus ada file pendukung
  ...
  // gunakan sumber DB yang konsisten
  const db =
    typeof globalThis !== "undefined" && globalThis.allPraktikumSoalDb
      ? globalThis.allPraktikumSoalDb
      : allPraktikumSoalDb;

  // pastikan list adalah array copy agar tidak merubah referensi tak sengaja
  ...
  // jaga konsistensi module-level map dan globalThis
  if (typeof globalThis !== "undefined") globalThis.allPraktikumSoalDb = db;
