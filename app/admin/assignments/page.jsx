/**
 * Halaman Penugasan Asesor (Admin)
 * * Halaman ini memungkinkan Admin LSP untuk:
 * 1. Melihat daftar asesi berdasarkan Skema dan Kelas.
 * 2. Menugaskan Asesor untuk Ujian Teori (per unit kompetensi).
 * 3. Menugaskan Asesor untuk Ujian Praktikum dan Unjuk Diri.
 * 4. Melakukan penugasan massal (Bulk Assign) per kelas dengan validasi konflik jadwal.
 * 5. Menyimpan penugasan ke database.
 */
...
/**
 * Helper untuk memformat daftar unit menjadi string yang mudah dibaca.
 * Contoh: "Unit 1", "Unit 2" dan "Unit 3".
 */
...
export default function AssignmentsPage() {
  ...
  // --- State Penugasan (Menyimpan pilihan user sementara sebelum disimpan) ---
  const [teoriAssignments, setTeoriAssignments] = useState({}) // Format Key: `${asesiId}-${unitId}`
  const [praktikumAssignments, setPraktikumAssignments] = useState({}) // Format Key: asesiId
  const [unjukDiriAssignments, setUnjukDiriAssignments] = useState({}) // Format Key: asesiId

  // --- State untuk Fitur Penugasan Cepat (Bulk Assign) ---
  ...
  // Muat data unit ketika skema berubah
  useEffect(() => {
    if (skemaId) {
      loadSkemaData()
    }
  ...
  // Filter asesi dan asesor ketika skema berubah
  useEffect(() => {
    if (!skemaId) return
  ...
  const loadSkemaList = async () => {
    try {
      ...
      console.error("Error memuat daftar skema:", error)
    } finally {
  ...
  const loadSkemaData = async () => {
    try {
      ...
      console.error("Error memuat data unit:", error)
    } finally {
      setLoading(false)
    }
  }

  // --- Memoized Data (Optimasi) ---

  // Mendapatkan daftar kelas unik dari asesi yang difilter
  const kelasList = useMemo(() => {
    const kelasSet = new Set(asesiList.map((a) => a.kelas).filter(Boolean))
    return Array.from(kelasSet).sort()
  }, [asesiList])

  // Opsi dropdown untuk unit kompetensi
  ...
  // Filter asesi berdasarkan pencarian dan kelas
  ...
  // Validasi: Apakah semua asesi yang tampil sudah memiliki asesor untuk semua unit (Teori)
  ...
  /**
   * Fungsi generik untuk menyimpan penugasan ke API
   */
  const handleSaveAssignments = async (asesiId, assignmentsData, asesiNama = null) => {
    ...
    try {
      ...
      console.error("Error menyimpan penugasan:", error)
  ...
  const handleSavePraktikum = (asesiId, asesiNama) => {
    ...
    handleSaveAssignments(asesiId, [{ tipe: "PRAKTIKUM", unitId: null, asesorId: asesorId }], asesiNama)
  }
  ...
  const handleSaveUnjukDiri = (asesiId, asesiNama) => {
    ...
    handleSaveAssignments(asesiId, [{ tipe: "UNJUK_DIRI", unitId: null, asesorId: asesorId }], asesiNama)
  }

  // --- Logika Penugasan Cepat (Bulk Assign) ---

  /**
   * Menerapkan penugasan ke state lokal untuk satu kelas penuh.
   */
  ...
  /**
   * Mengecek apakah ada konflik (asesor sudah dipilih sebelumnya) sebelum melakukan bulk assign.
   */
  const applyBulkAssign = (kelas, asesorId, setAssignmentsFunc, tipe) => {
    ...
    if (tipe === "TEORI") {
      ...
      // Jika tidak ada konflik, langsung eksekusi
      if (firstConflictAsesiId === null) {
        pendingAction();
        return;
      }

      // Jika ada konflik, tampilkan dialog konfirmasi
      ...
      // Untuk Praktikum & Unjuk Diri, langsung eksekusi (tidak ada cek unit overlap)
  ...
  const handleSaveAllTeori = async () => {
    ...
    try {
      ...
      let successMessage = filterKelas !== "SEMUA" 
        ? `Penugasan asesor di kelas ${filterKelas} berhasil disimpan untuk ${successCount} asesi.`
        : `Berhasil menyimpan ${successCount} asesi dari semua kelas yang tampil.`;
  ...
  const handleSaveAllPraktikum = async () => {
    ...
    try {
      for (const asesi of asesiToSave) {
        ...
        try {
          await mockAssignAsesorPerUnit(asesi.id, [{ tipe: "PRAKTIKUM", unitId: null, asesorId: asesorId }])
          successCount++
        } catch (error) {
          failCount++
        }
      ...
      const successMessage = filterKelas !== "SEMUA"
        ? `Penugasan praktikum di kelas ${filterKelas} berhasil disimpan untuk ${successCount} asesi.`
        : `Berhasil menyimpan ${successCount} asesi.`;
  ...
  const handleSaveAllUnjukDiri = async () => {
    ...
    try {
      for (const asesi of asesiToSave) {
        ...
        try {
          await mockAssignAsesorPerUnit(asesi.id, [{ tipe: "UNJUK_DIRI", unitId: null, asesorId: asesorId }])
          successCount++
        } catch (error) {
          failCount++
        }
      ...
      const successMessage = filterKelas !== "SEMUA"
        ? `Penugasan unjuk diri di kelas ${filterKelas} berhasil disimpan untuk ${successCount} asesi.`
        : `Berhasil menyimpan ${successCount} asesi.`;
  ...
  return (
    <MainLayout>
                ...
                <Select value={skemaId} onValueChange={setSkemaId} disabled={loadingSkema}>
                  ...
                  <SelectContent {...dropdownProps}>
                    {skemaList.length === 0 && !loadingSkema ? (
                      ...
                      skemaList.map((skema) => (
                        <SelectItem key={skema.id} value={skema.id}>{skema.judul} ({skema.id})</SelectItem>
                      ))
                    )}
                  </SelectContent>
                </Select>
          ...
          <CardContent className="p-0">
            {loading ? (
              ...
              <>
                    ...
                    <div className="p-4 border-b">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-6 gap-3 mb-4">
                        ...
                        <Select value={selectedBulkUnitStart} onValueChange={setSelectedBulkUnitStart}>
                          ...
                          <SelectContent {...dropdownProps}>
                            {unitOptions.map((u) => (
                              <SelectItem key={`${skemaId}-${u.value}`} value={u.value}>{u.label}</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                        ...
                        <Button
                          ...
                          title={!isAllFilteredAsesiFullyAssigned ? "Pastikan semua asesi di filter ini terisi penuh" : "Simpan semua penugasan"}
                        >
                          {savingAsesiId === "all" ? (
                      ...
                      <Table className="w-full">
                        <TableHeader className="bg-gray-50">
                          <TableRow>
                            ...
                            <TableHead className="text-center sticky right-0 bg-gray-50 z-10 min-w-[80px]">Aksi</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {paginatedAsesi.map((asesi) => {
                            const isThisAsesiSaving = savingAsesiId === asesi.id
                            const areAllUnitsAssigned = units.every((unit) => teoriAssignments[`${asesi.id}-${unit.id}`])
                ...
                {/* Tampilan untuk PRAKTIKUM dan UNJUK DIRI (Struktur serupa, disederhanakan di sini) */}
                {(tipeUjian === "PRAKTIKUM" || tipeUjian === "UNJUK_DIRI") && (
                  <div className="space-y-4">
                    <div className="p-4 border-b">
                      <h4 className="font-semibold text-blue-900 mb-3 flex items-center">
                        Penugasan Cepat {tipeUjian === "PRAKTIKUM" ? "Praktikum" : "Unjuk Diri"}
                      </h4>
                      {/* Form Penugasan Cepat untuk tipe non-teori... */}
                      {/* ... (Logika sama dengan TEORI tapi tanpa range unit) ... */}
                      <div className="flex gap-3 items-end flex-wrap mb-4">
                        <div className="flex-1 min-w-[200px]">
                          <Select 
                            value={tipeUjian === "PRAKTIKUM" ? selectedBulkClassPraktikum : selectedBulkClassUnjukDiri} 
                            onValueChange={tipeUjian === "PRAKTIKUM" ? setSelectedBulkClassPraktikum : setSelectedBulkClassUnjukDiri}
                          >
                        ...
                        <div className="flex-1 min-w-[200px]">
                          <Select 
                            value={tipeUjian === "PRAKTIKUM" ? selectedBulkAsesorPraktikum : selectedBulkAsesorUnjukDiri} 
                            onValueChange={tipeUjian === "PRAKTIKUM" ? setSelectedBulkAsesorPraktikum : setSelectedBulkAsesorUnjukDiri}
                          >
                        ...
                        <Button
                          onClick={tipeUjian === "PRAKTIKUM" ? handleBulkAssignPraktikum : handleBulkAssignUnjukDiri}
                          disabled={
                            tipeUjian === "PRAKTIKUM" 
                              ? (!selectedBulkClassPraktikum || !selectedBulkAsesorPraktikum) 
                              : (!selectedBulkClassUnjukDiri || !selectedBulkAsesorUnjukDiri)
                          }
                        >
                          Terapkan ke Kelas
                        </Button>
                        <Button
                          onClick={tipeUjian === "PRAKTIKUM" ? handleSaveAllPraktikum : handleSaveAllUnjukDiri}
                          variant="default"
                          className="bg-green-600 hover:bg-green-700"
                          disabled={
                            savingAsesiId === "all" || 
                            (tipeUjian === "PRAKTIKUM" ? !isPraktikumSaveAllValid : !isUnjukDiriSaveAllValid)
                          }
                        >
                          {savingAsesiId === "all" ? (
                            <><Spinner className="w-4 h-4 mr-2" /> Menyimpan...</>
                      ...
                      <Table className="w-full">
                        ...
                        <TableBody>
                          {paginatedAsesi.map((asesi) => {
                            ...
                            const currentAsesorId = tipeUjian === "PRAKTIKUM" 
                              ? praktikumAssignments[asesi.id] 
                              : unjukDiriAssignments[asesi.id] || ""
                              ...
                              <TableRow key={asesi.id} className="hover:bg-gray-50">
                                ...
                                <TableCell className="w-[40%]">
                                  <Select
                                    ...
                                    onValueChange={(value) => 
                                      tipeUjian === "PRAKTIKUM" 
                                        ? handlePraktikumAssignmentChange(asesi.id, value)
                                        : handleUnjukDiriAssignmentChange(asesi.id, value)
                                    }
                                  >
                                ...
                                <TableCell className="w-[25%] text-center">
                                  <Button
                                    ...
                                    onClick={() => 
                                      tipeUjian === "PRAKTIKUM" 
                                        ? handleSavePraktikum(asesi.id, asesi.nama)
                                        : handleSaveUnjukDiri(asesi.id, asesi.nama)
                                    }
      ...
      {/* Dialog Konfirmasi Overlap (Konflik Penugasan) */}
      <AlertDialog open={isOverlapDialogOpen} onOpenChange={setIsOverlapDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{overlapWarning}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleConfirmOverlap} className="bg-orange-600 hover:bg-orange-700">
      ...
      <AlertDialog open={successDialog.open} onOpenChange={(open) => setSuccessDialog({ ...successDialog, open })}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogAction onClick={() => setSuccessDialog({ open: false, message: "" })}>Lanjutkan</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={errorDialog.open} onOpenChange={(open) => setErrorDialog({ ...errorDialog, open })}>
        <AlertDialogContent className="sm:max-w-md">
          <AlertDialogHeader>
            <AlertDialogTitle>Validasi Unit</AlertDialogTitle>
            <AlertDialogDescription>{errorDialog.message}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogAction onClick={() => setErrorDialog({ open: false, message: "" })}>Lanjutkan</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
